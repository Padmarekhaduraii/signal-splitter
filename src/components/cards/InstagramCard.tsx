...
export function InstagramCard({
  ...
  const handleCopy = async () => {
    ...
    if (ok) {
      setIsCopied(true);
      success('Carousel copied!', 'Slides & caption ready to design in Canva or Figma.');
  ...
  const handleSaveEdit = () => {
    ...
    setIsEditing(false);
    success('Changes saved!', 'Instagram carousel updated.');
  };
      ...
      <div>
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-800/80 mb-4 flex-wrap gap-2">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-xl bg-gradient-to-tr from-amber-500/20 via-rose-500/20 to-purple-500/20 border border-purple-500/40 text-purple-400">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h4 className="font-bold text-white text-base">Instagram Carousel</h4>
                {output.isEdited && (
              ...
              <p className="text-xs text-slate-400">Slide-by-slide storyboard & visual cues</p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between bg-slate-900/60 p-2 rounded-xl border border-slate-800 mb-4 flex-wrap gap-2">
          ...
          <Button
            ...
            onClick={handleCopy}
          >
            {isCopied ? 'Copied' : 'Copy Carousel'}
          </Button>
        </div>

        {/* Custom Instructions Input */}
        {customPromptOpen && (
          <div className="mb-4 p-4 rounded-xl bg-purple-950/40 border border-purple-700/50 animate-in fade-in duration-200">
            <div className="flex items-center gap-2 mb-2 text-xs font-semibold text-purple-300">
              <Sparkles className="w-3.5 h-3.5" />
              <span>Refine Instagram Carousel Prompt</span>
            </div>
            <textarea
              rows={2}
              value={customInstructions}
              onChange={(e) => setCustomInstructions(e.target.value)}
              placeholder="e.g. Expand to 8 slides, focus on visual diagrams, make slide 3 highlight statistics..."
              className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 mb-2"
            />
        ...
        {isEditing ? (
          <div className="space-y-2 mb-4">
            <textarea
              ...
              className="w-full bg-slate-900/90 border border-purple-500/50 rounded-xl p-4 text-sm text-slate-100 font-sans leading-relaxed focus:outline-none focus:ring-2 focus:ring-purple-500/40 resize-y"
            />
          </div>
        ) : (
        ...
        <div className="flex items-center justify-between text-xs flex-wrap gap-2">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 text-purple-300">
              <Sparkles className="w-3.5 h-3.5" />
              <span>Retention Potential: {output.qualityScore || 95}/100</span>
            </div>
          </div>
            ...
            {output.evidence && output.evidence.length > 0 ? (
              output.evidence.map((ev, idx) => (
                <div key={idx} className="p-2.5 rounded-lg bg-slate-900/80 border border-slate-800 text-slate-300">
                  <div className="font-medium text-purple-300 mb-0.5">Claim: &quot;{ev.claim}&quot;</div>
